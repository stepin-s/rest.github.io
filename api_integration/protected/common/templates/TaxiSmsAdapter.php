...
abstract class TaxiSmsAdapter extends TaxiAdapter
{
    ...
    public function sendSms($phone, $typeId)
    {
        if ($smsCode = $this->smsValidator->generateSms($phone)) {
            ...
            if (!$this->sendRealSms($phone, $message)) {
                ...
                $info->text = "Ошибка отправки кода подтверждения, пожалуйста, позвоните администратору: +7 (495) 120 39 77";
                return $info;
    ...
    public function createOrder($fromCity, $fromStreet, $fromHouse, $fromHousing, $fromBuilding, $fromPorch, $fromLat, $fromLon, $toCity, $toStreet, $toHouse, $toHousing, $toBuilding, $toPorch, $toLat, $toLon, $clientName, $phone, $priorTime, $customCarId, $customCar, $carType, $carGroupId, $tariffGroupId, $comment, $isMobile, $additional, $promocode)
    {
        if (!empty($this->iphoneHackRegistrationTel) && ($phone == $this->iphoneHackRegistrationTel)) {
            $adapter = new TaxiAppleFakeAdapter();
            return $adapter->createOrder($fromCity, $fromStreet, $fromHouse, $fromHousing, $fromBuilding, $fromPorch, $fromLat, $fromLon, $toCity, $toStreet, $toHouse, $toHousing, $toBuilding, $toPorch, $toLat, $toLon, $clientName, $phone, $priorTime, $customCarId, $customCar, $carType, $carGroupId, $tariffGroupId, $comment, $isMobile, $additional, $name, $lastName, $secondName, $promocode);
        } else {
            return null;
        }
    }
