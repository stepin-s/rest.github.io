...
abstract class CUriModule {
  ...
  protected static function define_prefix() {
    ...
    //array_push($ar_prefix, "");//слэш на конце префикса

    self::$_prefix = self::$_site_dir . implode("/", $ar_prefix);
  ...
  protected function define_site_prefix() {
    if (!$this->set_site_prefix($this->get_site_prefix()))
  ...
  */
  ...
  protected static function get_path($postf, $create_file = false) { //file_put_contents создает файл
    $class_name = get_called_class();
  ...
  public function define_ajax_query() {
    ...
    if (!array_key_exists("new", $_POST) || !array_key_exists("old", $_POST) || !array_key_exists("path", $_POST))
      return false;
