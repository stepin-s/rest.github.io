    ...
    public function checkAccess($commandName, $params)
    {
       ...
       /* if (!empty($this->_rulles[$commandName])) {
            foreach ($this->_rulles[$commandName] as $rulle) {
                file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/lo.log', print_r($commandName, true), FILE_APPEND);
                file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/lo.log', print_r($params, true), FILE_APPEND);
