...
<style type="text/css">
    ...
    .orderInfo, #result_order, #result_order p{
...
<div class="span6" style="padding-left: 80px;">
	 <div class="loader" style="display: none;"><img src="<?= SITE_TEMPLATE_PATH ?>/i/loading.GIF" alt="" /></div>
    <?php
    ...
    if (isset($orderId) && $orderId!=''):
         ...
         */
        ?>
        <?php require $_SERVER['DOCUMENT_ROOT'] . '/api_integration/include/bitrix/templates/order_result_info.php'; ?>
    <?php else: ?>
        ...
        <form class="form-horizontal row-fluid" name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data">
                                    ...
                                    <div  class="control-group  <? if (($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'CITY_OTKUDA') || ($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'CITY_KUDA')) echo 'span3'; ?><? if (($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'FROM') || ($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'TO')) echo 'span9'; ?><?= ($small) ? ' span4 ' : '' ?> <? if ($propertyID != 'NAME') echo in_array($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'], $field_quickly) ? 'quickly' : 'no_quickly' ?>" style="<?= ($propertyID == 'NAME' && $i == 0) ? 'display:none;' : '' ?>">
                                        ...
                                        <div class="controls">
                                            ...
                                            switch ($INPUT_TYPE):
                                                    ...
                                                    for ($i = 0; $i < $inputNum; $i++) {
                                                        ...
                                                        if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["GetPublicEditHTML"][0] == "CIBlockPropertyDateTime") {
                                                            ...
                                                            $APPLICATION->IncludeComponent(
                                                                    'bitrix:main.calendar', 'calendar_templ', array(
                                                ...
                                                case "T":
                                                    for ($i = 0; $i < $inputNum; $i++) {
                                                        ...
                                                        <textarea placeholder="<? if (intval($propertyID) > 0): ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else: ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?>" id="FIELD_<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] ?>" class="zak_input_ span12 textarea-style" cols="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"] ?>" rows="<?$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"] ?>2" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]"><?= $value ?></textarea>
                                                    ...
                                                    for ($i = 0; $i < $inputNum; $i++) {
                                                        ...
                                                        if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'TIP'):
                                                            ...
                                                            while ($city = $db_city->GetNext()) {
                                                                ...
                                                                ?></select>
                                                        <? else: ?>
                                                            <input placeholder="<? if (intval($propertyID) > 0): ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else: ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?>" id="FIELD_<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] ?>" class="<? echo $first ? 'find-me-input ui-autocomplete-input span9' : 'zak_input_ span12'; ?>  <?= ($small) ? ' span8 ' : '' ?>" type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" value="<?= ($propertyID == 'NAME' && $i == 0) ? GetMessage("COLORS3_TAXI_ZAKAZ_S_SAYTA") : $value ?>" />
                                                        <? endif; ?>
                                                        ...
                                                        <? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"] == "DateTime"): ?>
                                                            <?
                                                            $APPLICATION->IncludeComponent(
                                                                    'bitrix:main.calendar', 'calendar', array(
                                                                'FORM_NAME' => 'iblock_add',
                                                                'INPUT_NAME' => "PROPERTY[" . $propertyID . "][" . $i . "]",
                                                                'INPUT_VALUE' => $value,
                                            ...
                                            <? if ($propertyID != 'NAME' && $first): ?>								
                                                <!--a href="#" id="find-me" class="find-me" style="cursor: pointer; color: rgb(0, 136, 204); border-bottom-width: 1px; border-bottom-style: dashed; border-bottom-color: rgb(0, 136, 204);">����� ����</a-->
                                                <?
                                                $first = false;
                                            endif;
                                            ?>
                                            <? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'FROM'): ?>
                                                <a title="Определить моё местоположение" href="#" id="find-me" class="find-me" style="cursor: pointer;float: right;margin:5px 5px 0 -24px;">
            ...
            <div class="control-group knopka">
                <div class="controls submit_controls">					
                    ...
                    <input class="btn rel call_me" type="submit" name="iblock_submit" id="send_order_form" value="<?= GetMessage("COLORS3_TAXI_ZAKAZATQ") ?>" />					
...
<?php require $_SERVER['DOCUMENT_ROOT'] . '/api_integration/include/bitrix/templates/after_template.php'; ?>