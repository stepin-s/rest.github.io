...
<? $this->setFrameMode(true); ?>
    ...
    <link href='http://fonts.googleapis.com/css?family=PT+Sans:400,700|PT+Sans+Narrow:400,700&subset=latin,cyrillic'
    ...
    <div class="popup" style="padding: 0; position: static; margin: 0; width: 100%; overflow: visible;">
        <h1>Перезвоните мне</h1>
		<a href="" class="sprite close"></a>
        ...
        <div class="modal-body">
            ...
            <div id="<?= 'ajax_form_' . $bxajaxid ?>">
                ...
                <form class="form-horizontal ajax_form" name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>"
                    ...
                    <? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])): ?>
                        ...
                        <? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
                            ...
                            <div class="field" style="<?= ($propertyID == 'NAME' && $i == 0) ? 'display:none;' : '' ?>">
                                <div
                                    class="field-label"><? if (intval($propertyID) > 0): ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else: ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?><? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])): ?>
                                        ...
                                        case "N":
                                            for ($i = 0; $i < $inputNum; $i++) {
                                                ...
                                                <input class="input-style" type="text"
                                                       ...
                                                       value="<?= ($propertyID == 'NAME' && $i == 0) ? GetMessage("COLORS3_TAXI_ZAKAZ_ZVONKA") : $value ?>"/>
                                            ...
                                            switch ($type):
                                                ...
                                                case "radio":
                                                    ...
                                                    foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum) {
                                                        ...
                                                        <label
                                                            for="property_<?= $key ?>"><?= $arEnum["VALUE"] ?></label>
                                                        <?
                                                    }
                                                    break;
                    ...
                    <div class="submit-field">
                        <input class="button yellow" type="submit" name="iblock_submit"
                               value="<?= GetMessage("IBLOCK_FORM_APPLY") ?>"/>
                    </div>
                    <? if (strlen($arParams["LIST_URL"]) > 0 && $arParams["ID"] > 0): ?><input type="submit"
                                                                                               name="iblock_apply"
...
<? /*
	<div class="submit-field">	
		<button style="display: none;" class="button yellow" type="button" name="iblock_submit"><?=GetMessage("COLORS3_TAXI_OTPRAVITQ")?></button>					
	</div>
*/ ?>